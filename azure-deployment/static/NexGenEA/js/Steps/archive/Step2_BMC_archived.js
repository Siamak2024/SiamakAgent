...
- Ground each item in company description - no generic filler`,
      ...
      userPrompt: (ctx) => {
        // Support both legacy (strategicIntent) and new (businessContext) data models
        ...
        if (mode === 'business-object' && ctx.businessContext) {
          // NEW: Business-object mode - use businessContext & businessObjectives
          const bc = ctx.businessContext;
          const objectives = ctx.businessObjectives || window.model?.businessObjectives || [];
          const themes = ctx.strategicThemes || window.model?.strategicThemes || [];
          
          // Extract keywords from objectives for context
          const objectivesSummary = objectives.map(obj => 
            `${obj.title} (${obj.category} - ${obj.priority}): ${obj.kpi} → ${obj.target_value} by ${obj.time_horizon}`
          ).join('\n');
          
          const themesSummary = themes.map(t => t.name).join(' | ');
          
          return `Company: "${ctx.companyDescription || bc.org_name || 'Organization'}"

Business Context (from Step 1):
- Industry: ${bc.industry || ''}
- Market: ${bc.market_summary || ''}
- Challenges: ${(bc.challenges || []).join('; ')}
- Constraints: ${(bc.constraints || []).join('; ')}

Strategic Themes: ${themesSummary}

Business Objectives:
${objectivesSummary}

Map the CURRENT state BMC. Ground each block in the company description and business context. Mark gaps with ⚠️.
...
- Show bold changes from current to future state`,
      ...
      userPrompt: (ctx) => {
        ...
        if (mode === 'business-object' && ctx.businessContext) {
          ...
          const objectives = ctx.businessObjectives || window.model?.businessObjectives || [];
          const themes = ctx.strategicThemes || window.model?.strategicThemes || [];
          const gaps = ctx.gapInsights || window.model?.gapInsights || [];
          
          // Extract keywords from objectives
          const objectivesSummary = objectives.map(obj => 
            `• ${obj.title} [${obj.category}/${obj.priority}]: ${obj.kpi} → ${obj.target_value} (${obj.time_horizon})\n  Rationale: ${obj.rationale}`
          ).join('\n');
          
          const themesSummary = themes.map((t, i) => `${i+1}. ${t.name}: ${t.description}`).join('\n');
          
          const gapsSummary = gaps.length > 0 
            ? `\n\nGap Insights:\n${gaps.map(g => `• ${g.type}: ${g.description}${g.recommendation ? ` → ${g.recommendation}` : ''}`).join('\n')}`
            : '';
...
- Market: ${bc.market_summary || ''}
- Challenges: ${(bc.challenges || []).join(' | ')}
- Value Drivers: ${(bc.value_drivers || []).join(' | ')}

Strategic Themes:
${themesSummary}

Business Objectives (CRITICAL - future BMC MUST align to these):
...
Design the FUTURE state BMC that enables these business objectives. Show bold shifts — not just refining the current model. Include transformation_moves to explain each major change.

CRITICAL: Every block should trace back to at least one business objective. Use the gap insights to identify where transformations are needed.
      ...
      userPrompt: (ctx) => {
        ...
        if (mode === 'business-object' && ctx.businessContext) {
          ...
          const objectivesContext = objectives.map(obj => 
            `${obj.title} [${obj.category}/${obj.priority}]`
          ).join('; ');
          
          return `Business Objectives: ${objectivesContext}
Strategic Themes: ${themes.map(t => t.name).join(' | ')}
Industry Context: ${bc.industry || ''} | Challenges: ${(bc.challenges || []).slice(0, 2).join(', ')}
...
Produce the delta analysis. Focus on how the BMC shifts enable the business objectives. executive_summary: 2-3 sentences for the Board.
